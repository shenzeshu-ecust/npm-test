...
export const ctsDateFromStr = function(stringDate, dateFormate) {
  _ctsDateFromString(stringDate, dateFormate);
}
...
const _ctsDateFromString = function(stringDate, dateFormate) {
  if (stringDate && stringDate.length > 0) {
    ...
    } else if (dateFormate === CTS_DATE_FORMATE.FORMATE_02 || dateFormate === CTS_DATE_FORMATE.FORMATE_04) {
...
export const ctsFormatStringForDate = function (date, dateFormat) {
  return _ctsFormatStringForDate(date, dateFormat);
}
...
const _ctsFormatStringForDate = function(date, dateFormate) {
  if (date) {
    ...
    } else if (dateFormate === CTS_DATE_FORMATE.FORMATE_04) {
      return _ctsFormat04StringWithDate(date);
...
const _ctsFormat04StringWithDate = function(date) {
  if (!date) {
    return null;
  }
  return date.getFullYear() + "年" + _ctsAppendZeroString(date.getMonth() + 1) + "月" + _ctsAppendZeroString(date.getDate() + "日");
}
