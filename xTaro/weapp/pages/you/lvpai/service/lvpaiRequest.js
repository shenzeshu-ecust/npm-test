...
function doBeforeRequest({url, data, success, fail, complete}) {
    ...
    let extensions = { head: {
        extension: [
            {"name": "GsMiniPlantform", "value": "wechatMiniapp"},
        ],
    } }
...
function secondLevelCommentList(param, fn) {
  ...
  if (typeof param.isNeedShowTag !== 'undefined') data.isNeedShowTag = param.isNeedShowTag;
  if (typeof param.isNeedInteractTag !== 'undefined') data.isNeedInteractTag = param.isNeedInteractTag;
  if (!__global.mock) {
    doBeforeRequest({
