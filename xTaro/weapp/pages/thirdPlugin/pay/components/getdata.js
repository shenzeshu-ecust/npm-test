...
function getServerData(orderDetailData={}) {
    ...
    if(!auth){
		...
		try{
			Business.sendUbtTrace({a:'getdata', c:100103, dd:'获取AUTH异常!', d:'获取cwx.user.auth的值为： ' + auth});
		}catch(e){}
        return;
    }else{
...
function buildServerData(response) {
    ...
    try {
        if(extendJson.titletype == 2){
            // 机票场景拼接子标题
        detailJson.subtitle = tmpData.customTitle.map(item=>`${item.tag}:${item.title} ${item.content}`).join('\n')
        }else{
        _.map(tmpData.customTitle, function(v, i){
            // 子标题选择customTitle的最后一个
    ...
    return {
        token: cwx.util.base64Encode(JSON.stringify(detailJson)),
        extend: cwx.util.base64Encode(JSON.stringify(extendJson)),
        unifiedData: true
    }
}
...
function getData(callback){
        ...
        try {
			...
			try{
				Business.sendUbtTrace({a:'getdata', c:1001, dd:'开始解析BU参数', d:'getdata start!'});
            }catch(e){}
        ...
        if (dataError) {
			...
			try{
				Business.sendUbtTrace({a:'getdata', c:100103, dd:'Bu传递url解析异常!', d:'BU传过来的参数token: ' + payData.token + '; extend: ' + payData.token});
			}catch(e){}
            return;
        }
