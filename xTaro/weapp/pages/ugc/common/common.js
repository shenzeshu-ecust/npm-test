...
function delUrlParam(url, ref) { //删除url里的参数
...
function webviewHomeConfig(){
  ...
  if ( __global.env.toLowerCase() === 'uat') {
    url = 'https://m.uat.qa.nt.ctripcorp.com/webapp/you/tripshoot/paipai/home/home?isMini=2&disable_redirect_https=1&seo=0';
  }else{
    url = 'https://m.ctrip.com/webapp/you/tripshoot/paipai/home/home?isMini=2&seo=0';
  }
  param = {
...
function newUrlPrefix() {
  ...
  if (__global.env.toLowerCase() === 'uat') {
    if (ISLOCAL) {
      ...
      prefix = 'https://m.ctrip.fat325.qa.nt.ctripcorp.com/webapp/you/tripshoot/paipai/'
  } else {
...
function checkResponseAck(res,callback,toastFlag){
  var status = false;
  if(res && res.statusCode == 200 && res.data && res.data.ResponseStatus && res.data.ResponseStatus.Ack.toLowerCase() === "success"){
    status = true;
...
function checkSessionOverTime(res){
  var status = true;  //未过期
  if(res && res.data && res.data.ResponseStatus && res.data.ResponseStatus.Ack.toLowerCase() === "failure" && res.data.ResponseStatus.Errors[0].ErrorCode == "MobileRequestFilterException"){
    status = false;  //已过期
  }
  return status;
}
