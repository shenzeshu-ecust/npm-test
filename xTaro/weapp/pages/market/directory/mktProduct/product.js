import { apiServer, filterProps } from "./../legaoComponents/productCanvas/utils/api"
import { cwx, _ } from "./../../../../cwx/cwx";
import { goTargetUrl } from "./../../common/utils"
const behavior = require('./../legaoComponents/productCanvas/utils/behavior');
const isArray = (arg) => {
...
Component({
  ...
  methods: {
    ...
    loadMore() {
      ...
      let startIndex = renderDataList.length <= 2000 ? 0 : 2000 * this.data.listProduct.length;
      let endIndex = renderDataList.length <= 2000 ? 2000 : 2000 * (this.data.listProduct.length + 1);
      // let startIndex = 0
      // let endIndex = 10000
    ...
    incrementalUpdate() {
      ...
      if (updateType === "update") {
        let newListProduct = listProduct.map((item, index) => {
          let obj = []
          if (item) {
            obj = renderDataList.slice(2000 * index, 2000 * (index + 1))
            obj.forEach(i => {
    ...
    clickHandlers(options) {
      try {
        ...
        goTargetUrl(detail?.product?.wxUrl || detail?.product?.h5Url);
