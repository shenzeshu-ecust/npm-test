...
CPage({
  ...
  onLoad: function (options) {
    ...
    if(!taskinvite) {
      UTILS.showToast('缺少路由参数 taskinvite')
      return
    }
  ...
  computeTaskBtn: function(taskList, inviteInfo) {
    ...
    if (taskList.length == 0) {
      return {
        status: 3,
        btnText: this.data.formSceneInfo?.extension?.joinedText || '之前已参与，我也要发起'
      }
    }
    ...
    if (currentTask.status == 3) {
      return {
        status: 2,
        btnText: taskList[0].buttonDoneText || '已完成'
      }
    }
    
    return {
      status: 0,
      btnText: taskList[0].buttonText || '去完成'
  }
  },
