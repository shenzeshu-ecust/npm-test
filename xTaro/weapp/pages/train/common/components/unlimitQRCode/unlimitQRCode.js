...
export const unlimitQRCode = {
    ...
    onLoad(options) {
        let optionScene = options.scene ? decodeURIComponent(options.scene) : ""
    ...
    goTargetUrl(targetUrl) {
        ...
        if ((targetUrl.indexOf('pages/home/homepage') !== -1) || (targetUrl.indexOf('pages/myctrip/list/list') !== -1) || (targetUrl.indexOf('pages/myctrip/tablist/tabAllList') !== -1)) {
            //针对tab切换页，手动写入业绩
            let targetQuery = decodeURIComponent(targetUrl)
            cwx.mkt.setUnion({
            ...
            cwx.redirectTo({
                url: "/" + targetUrl.trim(),
                fail: function (e) {
                    that.ubtTrace(101066, {targetUrl: targetUrl, errmsg: "错误信息：" + (e.errMsg || e)})
                    that.goHome()
                },
            })
        } else {
