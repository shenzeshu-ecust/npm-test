...
module.exports = {
	...
	setParams: function (url, params) {
		...
		mergeParams = this.param(currentParams);
	...
	checkIsFullUrl: function(url) {
		if (!url) return false
		const containArray = ['http', 'ctrip.com', 'ctripcorp.com']
		return containArray.some(c => url.indexOf(c) > -1)
	}
};
