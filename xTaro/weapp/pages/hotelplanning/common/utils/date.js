...
export default {
	...
	compareTime: function(date, otherDate) {
		const dateTime = date?.getTime() || 0;
		const otherDateTime = otherDate?.getTime() || 0;
		return otherDateTime - dateTime;
	},
	getTimeOfToday: function(hour) {
		 const now = parse(this.today());
		 return new Date(now.getTime() + hour * 3600000);
	},
	calDays: function (inDay, outDay) {
		return Math.round(
			(parse(outDay).getTime() - parse(inDay).getTime()) / 86400000
		);
	},
